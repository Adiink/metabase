...
(deftest create-tables-if-not-exists!-test
  (let [pgvector       semantic.tu/db
        index-metadata (semantic.tu/unique-mock-index-metadata)
    ...
    (testing "is idempotent when tables already exist"
      (with-open [_ (sut pgvector index-metadata)]
        (let [table-names-snap (get-table-names pgvector)
              _                (sut pgvector index-metadata)]
          (is (= table-names-snap (get-table-names pgvector))))))))
  ...
  (semantic.tu/closeable
   ...
   (fn [_]
     (doseq [{:keys [table_name]}
             (when (semantic.tu/table-exists-in-db? (:metadata-table-name index-metadata))
    ...
    (with-open [_ (open-tables pgvector index-metadata)]
      (let [table-names-snap (get-table-names pgvector)
            ...
            now-expected     (remove (hash-set (:control-table-name index-metadata)
                                     ...
                                     table-names-snap)]
        (is (= now-expected (get-table-names pgvector)))))))
  ...
  (let [pgvector       semantic.tu/db
    ...
    (with-open [_ (open-tables pgvector index-metadata)]
      (testing "creates singleton control row with id=0"
        (is (= [] (get-control-rows pgvector index-metadata)) "should be empty at start")
        (sut pgvector index-metadata)
        (is (= [{:id 0, :version (:version index-metadata), :active_id nil, :active_updated_at nil}]
               (get-control-rows pgvector index-metadata))))
      (testing "is idempotent - doesn't create duplicate rows"
        (let [control-snap (get-control-rows pgvector index-metadata)]
          (sut pgvector index-metadata)
          (is (= control-snap (get-control-rows pgvector index-metadata))))))))
    ...
    (with-open [_ (open-tables pgvector index-metadata)]
      (let [index-id1 (semantic.index-metadata/record-new-index-table! pgvector index-metadata index1)
        ...
        (testing "does nothing if no control row"
          ...
          (is (= [] (get-control-rows pgvector index-metadata))))

        (semantic.index-metadata/ensure-control-row-exists! pgvector index-metadata)

        (testing "sets specified index as active"
          (sut pgvector index-metadata index-id1)
          (is (=? [{:active_id index-id1}] (get-control-rows pgvector index-metadata)))
          (testing "switch again"
            (sut pgvector index-metadata index-id2)
            (is (=? [{:active_id index-id2}] (get-control-rows pgvector index-metadata)))))))))
    ...
    (with-open [_ (open-tables pgvector index-metadata)]
      ...
      (testing "returns active index configuration when one is set"
        (let [index-id (semantic.index-metadata/record-new-index-table! pgvector index-metadata index)]
          ...
          (is (=? [{:active_id index-id}] (get-control-rows pgvector index-metadata)))
          (is (=? {:index        index
                   :metadata-row (first (get-metadata-rows pgvector index-metadata))}
                  (sut pgvector index-metadata))))))))
...
(deftest find-best-index!-test
  (let [pgvector         semantic.tu/db
        ...
        ;; extend to table not existing or metadata not existing might be good
    ...
    (doseq [{:keys [desc active inactive] :as scenario} scenarios
            :let [index-metadata (semantic.tu/unique-mock-index-metadata)
      ...
      (testing (str "scenario: " desc)
        (with-open [_ (open-tables pgvector index-metadata)]
          ...
          (let [metadata-rows   (get-metadata-rows pgvector index-metadata)
  ...
  (let [pgvector        semantic.tu/db
    ...
    (with-open [_ (open-tables pgvector index-metadata)]
      (testing "empty to start"
        (is (= [] (get-metadata-rows pgvector index-metadata))))
